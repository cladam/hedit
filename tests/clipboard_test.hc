...
test "clipboard helpers preserve the fallback when commands fail" {
  let broken = Some(("hedit-missing-clipboard-command", "hedit-missing-clipboard-command"))
  assert(os_clipboard_set(broken, "outside") == false)
  assert_eq(os_clipboard_get(broken, "inside"), "inside")
}
